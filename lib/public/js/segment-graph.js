...
function onMouseDown(event){
  ...
  if(isTargetLine(hit.item.name)){
    var cue_idx = cueIDX(hit.item.name); 
    var target_idx  = targetIDX(hit.item.name);
    editKeyFrameEase(target_idx,cue_idx);
    return;
  }
}
...
function cueIDX(cue_name){
  ...
  if(cue_name.indexOf('L') == 0){
    n = cue_name.replace('L_','');
  }
  n = n.split('_')[0];
  return parseInt(n);
}
...
function renderTarget(target_id,cuePoints,keyFrames,color,visibility){
  ...
  for(var i = 0;i < cuePoints.length;i++){
    ...
    if(prev_point !== null){
      ...
      var rc = new Rectangle(prev_point,p);
      var h1 = null;
      var h2 = null;
     
      //create a 'wiggly' line to hint the nonlinearity 
      if(keyFrame.easing && keyFrame.easing != 'linear'){
        h1 = rc.topCenter - prev_point;
        h2 = rc.bottomCenter - p;
      }

      var seg1 = new Segment(prev_point, null, h1);
      var seg2 = new Segment(p, h2, null);
