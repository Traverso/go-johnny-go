...
var Segment = function(name){
  ...
  this.keyFrames = [ [{degrees:0,active:false},{degrees:45},{degrees:130},{degrees:45},{degrees:0}] ];
  this.loop = false;
};
...
Animax.prototype.selectedCue = function(cue_idx){
    ...
    var keys = this.currentSegment.keyFrames;
    for(var i = 0; i < keys.length;i++){

      if(!this.currentSegment.targets[i].visibility){
        continue;
      }

      if(keys[i][cue_idx].active == false){
        continue;
      }
      
      var target = JSON.parse(JSON.stringify(this.currentSegment.targets[i]));
      target.startAt = keys[i][cue_idx].degrees;

      animation.targets.push(target);
      animation.keyFrames.push([null,keys[i][cue_idx]]);
    }
...
Animax.prototype.toggleKeyFrameActivity = function(target_idx,cue_idx){
  ...
  this.save();
  return { active:current_status,color:this.currentSegment.targets[target_idx].color };
};
...
Animax.prototype.exportTargets = function(segment){
  var targets = [];
  for(var i = 0; i < segment.targets.length;i++){
    if(!segment.targets[i].visibility) continue;
    targets.push(JSON.parse(JSON.stringify(segment.targets[i])));
  }
  return targets;
};
...
Animax.prototype.exportKeyFrames = function(segment){
  ...
  for(var i = 0; i < segment.keyFrames.length;i++){
    ...
    if(!segment.targets[i].visibility){
      continue;
    }
...
Animax.prototype.exportSegmentTMP = function(segment){
  ...
  return {
               ...
               loop:segment.loop,
               targets:this.exportTargets(segment)
            }; 
};
