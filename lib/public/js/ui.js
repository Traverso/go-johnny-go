...
$(document).ready(function() {
    ...
    $('#save-easing').click(function(){
      animax.saveEasingForKeyFrame( $('#keyframe-target-idx').val(),
                                    $('#keyframe-cue-idx').val(),
                                    $('#keyframe-easing').val());

      $('#easing-modal').modal('hide');
      animax.loadSegmentGraph();
    });
    ...
    $("#playlist-loop").click(function(){
        if($(this).attr('aria-pressed') == 'false'){
            $('span','#playlist-loop').removeClass('glyphicon-off');
            $('span','#playlist-loop').addClass('glyphicon-refresh');
            animax.updatePlaylistLoop(true);
        } else {
            $('span','#playlist-loop').removeClass('glyphicon-refresh');
            $('span','#playlist-loop').addClass('glyphicon-off');
            animax.updatePlaylistLoop(false);
        }
    });
    ...
    for(var i = 0; i < Eases.length; i++){
      $('.easing-list').append('<option>'+ Eases[i] +'</option>');
    }
});
...
function editProject(project_id){
    ...
    if(animax.currentProject.playlist.loop){
      $("#playlist-loop").attr('aria-pressed','true');
      $('span','#playlist-loop').removeClass('glyphicon-off');
      $('span','#playlist-loop').addClass('glyphicon-refresh');
    } else {
      $("#playlist-loop").attr('aria-pressed','false');
      $('span','#playlist-loop').removeClass('glyphicon-refresh');
      $('span','#playlist-loop').addClass('glyphicon-off');
    }
}
...
function editKeyFrameEase(target_idx,cue_idx){
   //get current ease
   var easing = animax.easingForKeyFrame(target_idx,cue_idx); 
   $('#keyframe-target-idx').val(target_idx);
   $('#keyframe-cue-idx').val(cue_idx);
   $('#keyframe-easing').val(easing);
   $('#easing-modal').modal();
}
